...
import cardPng from "../../../public/Assets/cardImg.jpg";
    ...
    <>
      <Box sx={style.mainBox}>
        <Container sx={style.container}>
          <Box
            sx={{ display: "flex", flexDirection: "row", alignItems: "center" ,paddingTop:'10px'}}
          >
          ...
          <Box sx={{display:"flex",flexDirection:"row",overflowX:"scroll",gap:"12px",width:"100%",paddingBottom:'5px'}} className='style-3'>
              ...
              {options?.OptionsList?.map((item, index) => {
                ...
                return (
                    <motion.div
                    initial={ animate && showDone.includes(String(item.ID)) ? { opacity: 0, scale: 2 } : {  scale: 1 } }
                    animate={animate && showDone.includes(String(item.ID)) ? { opacity: 1, scale: 1 } : {  scale: 1 } }
